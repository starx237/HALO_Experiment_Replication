...
class SelfAttention(nn.Module):
	...
	def forward(self, x):
		...
		out = torch.einsum('b c f, b f k -> b c k', proj_value, weights)
...
class DecoderConv(nn.Module):
	def __init__(self, h_dim, 
		...
		self.dec = nn.Sequential(
				...
				nn.BatchNorm2d(256),
				nn.LeakyReLU(0.2),
				nn.Conv2d(256, self.out_channels, 3, 1, 1),
				nn.BatchNorm2d(self.out_channels),
				nn.Tanh(),
			)
...
class DecoderResNet(nn.Module):
	def __init__(self, rows,
		...
		self.dec_fc.append(nn.Sequential(
						nn.Linear(h_dim, h_dim*int(rows/32)*int(columns/32)),
						Rearrange('b (c w h) -> b c w h', c=h_dim, w=int(rows/32), h=int(columns/32)),
						))
...
class DynamicalModel(nn.Module):
	...
	def matrix_exp(self, x, order=10, usetorch=False):
		if usetorch:
			...
			n, d, _ = x.shape
			I = torch.eye(d, device=x.device).unsqueeze(0).expand(n, d, d)
			out = I.clone()
			power = I.clone()
			for i in range(order):
				power = torch.bmm(power, x) / (i + 1)
				out = out + power
			return out
