...
from model import VAE, SubspaceVAE
#, Discriminator_V
	...
	def reconstruct_all_actions(self, x_t):
		...
		for i, x_i in enumerate(x_t):
			...
			if self.config['model']['useZ']:
				...
				sz = rearrange(s_t_i_gen[:,:,:,:subspace_dim], 'b t n v -> b t (n v)')
...
class TrainerAE(nn.Module):
	...
	def resume(self, ckpt, device=torch.device('cpu')):
