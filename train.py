	...
	for epoch in range(start, config['trainer']['num_epochs']):
		...
		for i, batch in enumerate(train_loader, 0):
			...
			if isinstance(batch, (list, tuple)):
				data_in, labels = batch
			else:
				data_in = batch['images'].permute(0,2,1,3,4)
...
if __name__ == '__main__':
	...
	if opt.dataname == 'mug':
		...
		# 参考代码（需先实现 mugdataset / VideoDataset 模块）:
		# dataset_train = MUGDataset(config['mug'], 'Train')
		# train_loader = DataLoader(VideoDataset(...), ...)
		...
		assert 0,f"{opt.dataname} Not Implemented"
